...
export const QuoteEditor: React.FC<{ block: ContentBlock }> = ({ block }) => {
    ...
    // Fixed text size for consistent look
    ...
    // Simple Style (Clean, Teko, Left Border)
    if (!isCard) {
        return (
    ...
    // Card Style (Driver Block Aesthetic)
    return (
        <div className={`relative group/quote ${widthClass} ${alignClass}`}>
            <div className="bg-f1-card rounded-2xl overflow-hidden shadow-2xl border border-white/5 relative p-10 md:p-12 font-display">
                ...
                <QuoteIcon 
                    className="absolute -top-6 -left-6 text-white/5 transform rotate-12 scale-150 pointer-events-none" 
                    size={180} 
                />
                ...
                <div className="relative z-10 flex flex-col">
                    ...
                    <div className="mt-8 flex flex-col items-center justify-center">
                        <div className="h-1 w-12 bg-f1-pink rounded-full mb-3 shadow-[0_0_10px_rgba(225,0,89,0.5)]"></div>
                        <input
                            value={block.attributes.citation || ''}
                            onChange={(e) => updateBlock(block.clientId, { citation: e.target.value })}
